...
def _chunk_cumsum_fwd_kernel(
    ...
    A_scale_ptr,  # PoST: optional per-position A scaling (batch, seqlen, nheads)
    # Matrix dimension
    ...
    stride_A_scale_batch, stride_A_scale_seqlen, stride_A_scale_head,
    # Meta-parameters
    ...
    # PoST: optionally scale A per position
    if HAS_A_SCALE:
        A_scale_ptr += pid_b * stride_A_scale_batch + pid_c * chunk_size * stride_A_scale_seqlen
        A_scale_ptrs = A_scale_ptr + (offs_h[:, None] * stride_A_scale_head + offs_c[None, :] * stride_A_scale_seqlen)
        A_scale = tl.load(A_scale_ptrs, mask=(offs_h[:, None] < nheads) & (offs_c[None, :] < chunk_size_limit), other=1.0).to(tl.float32)
        dA = dt * A[:, None] * A_scale
    else:
        dA = dt * A[:, None]
...
def _chunk_cumsum_bwd_kernel(
    ...
    A_scale_ptr, dA_scale_ptr,  # PoST: optional A scaling and its gradient
    # Matrix dimensions
    batch, seqlen, nheads, chunk_size,
    dt_min, dt_max,
    # Strides
    ...
    stride_A_scale_batch, stride_A_scale_seqlen, stride_A_scale_head,
    stride_dA_scale_batch, stride_dA_scale_seqlen, stride_dA_scale_head,
    # Meta-parameters
    DT_SOFTPLUS: tl.constexpr,
    HAS_DT_BIAS: tl.constexpr,
    HAS_A_SCALE: tl.constexpr,
    ...
    A = tl.load(A_ptrs, mask=offs_h < nheads, other=0.0).to(tl.float32)
    # PoST: load A_scale if present
    if HAS_A_SCALE:
        A_scale_base = A_scale_ptr + pid_b * stride_A_scale_batch + pid_c * chunk_size * stride_A_scale_seqlen
        A_scale_ptrs = A_scale_base + (offs_h[:, None] * stride_A_scale_head + offs_c[None, :] * stride_A_scale_seqlen)
        A_scale = tl.load(A_scale_ptrs, mask=(offs_h[:, None] < nheads) & (offs_c[None, :] < chunk_size_limit), other=1.0).to(tl.float32)
        A_eff = A[:, None] * A_scale
    else:
        A_eff = A[:, None]
    ddt = ddA * A_eff + ddt_out
    dt = tl.load(dt_ptrs, mask=(offs_h[:, None] < nheads) & (offs_c[None, :] < chunk_size_limit), other=0.0).to(tl.float32)
    if HAS_DT_BIAS:
        dt_bias = tl.load(dt_bias_ptr + offs_h * stride_dt_bias_head, mask=offs_h < nheads, other=0.0).to(tl.float32)
        dt += dt_bias[:, None]
    ...
    # PoST: gradient for A_scale = ddA * dt * A  (per position)
    if HAS_A_SCALE:
        dA_scale_val = ddA * dt * A[:, None]
        dA_scale_base = dA_scale_ptr + pid_b * stride_dA_scale_batch + pid_c * chunk_size * stride_dA_scale_seqlen
        dA_scale_store_ptrs = dA_scale_base + (offs_h[:, None] * stride_dA_scale_head + offs_c[None, :] * stride_dA_scale_seqlen)
        tl.store(dA_scale_store_ptrs, dA_scale_val, mask=(offs_h[:, None] < nheads) & (offs_c[None, :] < chunk_size_limit))
        # dA with A_scale: sum over positions of ddA * dt * A_scale
        dA = tl.sum(ddA * dt * A_scale, axis=1)
    else:
...
def _chunk_cumsum_fwd(dt, A, chunk_size, dt_bias=None, dt_softplus=False, dt_limit=(0.0, float("inf")), A_scale=None):
    batch, seqlen, nheads = dt.shape
    assert A.shape == (nheads,)
    if dt_bias is not None:
        assert dt_bias.shape == (nheads,)
    if A_scale is not None:
        assert A_scale.shape == (batch, seqlen, nheads)
    ...
    with torch.cuda.device(dt.device.index):
        _chunk_cumsum_fwd_kernel[grid_chunk_cs](
            ...
            A_scale.stride(2) if A_scale is not None else 0,
            dt_softplus,
...
def _chunk_cumsum_bwd(ddA, ddt_out, dt, A, dt_bias=None, dt_softplus=False, dt_limit=(0.0, float("inf")), ddt=None, A_scale=None):
    batch, seqlen, nheads = dt.shape
    ...
    # PoST: allocate dA_scale if needed
    dA_scale = torch.empty_like(A_scale, dtype=torch.float32) if A_scale is not None else None
    ...
    with torch.cuda.device(dt.device.index):
        _chunk_cumsum_bwd_kernel[grid_chunk_cs](
            ...
            A_scale.stride(1) if A_scale is not None else 0,
            A_scale.stride(2) if A_scale is not None else 0,
            dA_scale.stride(0) if dA_scale is not None else 0,
            dA_scale.stride(1) if dA_scale is not None else 0,
            dA_scale.stride(2) if dA_scale is not None else 0,
            dt_softplus,
            HAS_DT_BIAS=dt_bias is not None,
            HAS_A_SCALE=A_scale is not None,
            BLOCK_SIZE_CHUNK=triton.next_power_of_2(chunk_size),
    ...
    return ddt, dA, ddt_bias, dA_scale
