...
class MQARConfig(DataSegmentConfig):
    ...
    test_type: str = ""  # optional label for distinguishing test conditions in wandb

    def build(self, seed: int) -> DataSegment:
        extra_slices = {"test_type": self.test_type} if self.test_type else {}
        return multiquery_ar(**self.model_dump(exclude={"test_type"}), seed=seed, extra_slices=extra_slices)
