...
def build_optimizer_groups(model: nn.Module, base_lr: float, weight_decay: float):
    """
    Build AdamW parameter groups with selective weight decay (all architectures).

    Applies the standard GPT/RWKV-7 heuristic universally:
      - lr_2x (no WD): w0_base / w0_delta  → 2× LR  (PoST params only)
      - lr_decay (WD): p.squeeze().ndim >= 2  AND  ".weight" in name
      - lr_1x (no WD): everything else (biases, 1-D scalars, LN/GN params)

    The dimension heuristic naturally handles all architectures:
      Mamba-2:  A_log (1-D), D (1-D), dt_bias (1-D) → no WD ✓
      RWKV-7:   x_r/x_w/k_k/r_k (1-D or no .weight) → no WD ✓
      GLA/RetNet: dt_bias (1-D) → no WD ✓
    """
    ...
    for scale, params in by_scale.items():
        groups.append({"params": params, "weight_decay": 0.0, "lr": base_lr * scale})
...
class Trainer:
    ...
    def fit(self):
        ...
        self.use_amp = self.device.startswith('cuda') if isinstance(self.device, str) else (self.device.type == 'cuda')
        self.loss_fn = nn.CrossEntropyLoss()
        param_groups = build_optimizer_groups(
            self.model, self.learning_rate, self.weight_decay
        )
        self.optimizer = optim.AdamW(param_groups, lr=self.learning_rate)
        if self.lr_scheduler_type == "linear_decay":
            # Linear decay: lr drops linearly so last epoch has lr_max / max_epochs
            self.scheduler = optim.lr_scheduler.LambdaLR(
                self.optimizer,
                lr_lambda=lambda epoch: (self.max_epochs - epoch) / self.max_epochs,
            )
        else:
