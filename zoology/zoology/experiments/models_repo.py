...
def add_gated_delta_net(models, conv_mixer, input_seq_len, model_factory_kwargs, num_layers=2):
    block_type = "TransformerBlock"
    # NHEADS_BY_DMODEL matches Mamba-2/RWKV-7; GDN doubles heads to equalize state with expand_v=2
    nheads_by_d = {512: 8, 256: 4, 128: 4}
    for d_model in [128, 256, 512]:
        base_nheads = nheads_by_d[d_model]
        gdn_nheads = base_nheads * 2  # state = 2d²/(2h) = d²/h (matches RWKV-7)
        delta_net_mixer = dict(
            name="zoology.mixers.gated_delta_net.GatedDeltaNet",
            kwargs={
                "l_max": input_seq_len,
                "num_heads": gdn_nheads,
                "expand_v": 2,
                "use_gate": False,
                "use_short_conv": True,
                "conv_size": 4
...
# Gated DeltaNet PoST
def add_gated_delta_net_post(models, conv_mixer, input_seq_len, model_factory_kwargs, num_layers=2):
    block_type = "TransformerBlock"
    nheads_by_d = {512: 8, 256: 4, 128: 4}
    for d_model in [128, 256, 512]:
        base_nheads = nheads_by_d[d_model]
