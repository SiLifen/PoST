    ...
    def forward(
        ...
        **kwargs: Unpack[Dict]
        ...
        if mode == 'chunk':
            ...
            # installed fla's fused_recurrent_gated_delta_rule expects
            # head-first layout (B, H, T, D) and does not accept head_first kwarg.
            q_hf = rearrange(q, 'b t h d -> b h t d')
            k_hf = rearrange(k, 'b t h d -> b h t d')
            v_hf = rearrange(v, 'b t h d -> b h t d')
            beta_hf = rearrange(beta, 'b t h -> b h t')
            g_hf = rearrange(g, 'b t h -> b h t')
