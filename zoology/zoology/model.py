...
def _init_weights(
    ...
    if 'Mamba' in block_type or 'PoST' in block_type or 'RWKV' in block_type:
            ...
            if rescale_prenorm_residual:
                ...
                for name, p in module.named_parameters():
                    if name in ["out_proj.weight", "fc2.weight"]:
                        if not getattr(p, "_no_reinit", False):
...
class LMBackbone(nn.Module):
    def __init__(self, config: ModelConfig):
        ...
        elif config.block_type == "Mamba2PoSTBlock":
            from zoology.mixers.post_mamba2_mixer import Mamba2PoSTBlock
            block_cls = Mamba2PoSTBlock
        elif config.block_type == "RWKV7PoSTBlock":
            from zoology.mixers.post_rwkv7_mixer import RWKV7PoSTBlock
            block_cls = RWKV7PoSTBlock
        elif config.block_type == "RWKV7Block":
            from zoology.mixers.rwkv7 import RWKV7Block
        ...
        self._rwkv_blocks = config.block_type in ("RWKV7Block", "RWKV7PoSTBlock")
    ...
    def layers_forward(self, hidden_states):
        ...
        for layer in self.layers:
            result = layer(hidden_states, residual, v_first) if self._rwkv_blocks else layer(hidden_states, residual)
            if len(result) == 3:
                hidden_states, residual, v_first = result
            else:
...
def _compute_state_size(layers, sequence_length: int):
    ...
    for layer in layers:
        ...
        if Mamba2Block and isinstance(layer, Mamba2Block):
            mixer = layer.mixer
        elif Mamba2PoSTBlock and isinstance(layer, Mamba2PoSTBlock):
            mixer = layer.mixer
        elif RWKV7PoSTBlock and isinstance(layer, RWKV7PoSTBlock):
            mixer = layer.mixer
        elif RWKV7Block and isinstance(layer, RWKV7Block):
            mixer = layer.mixer
