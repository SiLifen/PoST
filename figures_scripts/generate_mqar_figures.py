#!/usr/bin/env python3
"""
Generate publication-quality MQAR figures for the PoST paper.

For each model dimension d ∈ {256, 128, 64}, produces one line plot showing
capacity accuracy (kv = T/4) at T ∈ {512, 1024, 2048, 4096}.

Data is fetched from wandb and cached locally.

Usage:
  python generate_mqar_figures.py               # fetch from wandb + plot
  python generate_mqar_figures.py --use-cached   # skip wandb, use cached json
"""
...
# Keyed by state-size label; each entry resolves to a unique (dim, nheads) pair.
# models list: (display_name, run_prefix, lr_sweep, model_type|None)
_MODELS = [
...
# Ordered 64K → 32K → 16K (largest state first)
STATE_CONFIGS = {
...
def plot_mqar(ax, seq_lens, model_data, ylabel="Accuracy", ylim=(-0.02, 1.05)):
    """Plot accuracy vs seq_len for available models on a single axis."""
    ...
    for model_name in PLOT_ORDER:
        ...
        if data is None:
            continue
        accs = [data.get(sl) for sl in seq_lens]
        if all(a is None for a in accs):
            continue

        accs = [a if a is not None else np.nan for a in accs]
...
def generate_figures(all_data: dict):
    ...
    all_best = {}  # state_label -> {model_name -> result}
    ...
    for state_label in state_order:
        ...
        plot_mqar(ax, SEQ_LENS, cap_data)
        ax.set_title(f"MQAR  ($d={cfg['dim']}$,  state$={state_label}$)")
        fig.tight_layout()
        for ext in ("pdf", "png"):
            fig.savefig(os.path.join(FIG_DIR, f"mqar_state{state_label}.{ext}"))
        print(f"  ✅ mqar_state{state_label}\n")
        plt.close(fig)
    ...
    for i, state_label in enumerate(states_with_data):
        ...
        cfg  = STATE_CONFIGS[state_label]
        cap_data = {m: r["capacity_per_len"] if r else None for m, r in best.items()}
        plot_mqar(ax, SEQ_LENS, cap_data, ylabel="Accuracy" if i == 0 else "")
    ...
    for ext in ("pdf", "png"):
        fig.savefig(os.path.join(FIG_DIR, f"mqar_combined.{ext}"))
    print(f"  ✅ mqar_combined\n")
    plt.close(fig)
